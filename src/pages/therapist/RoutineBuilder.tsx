...
export function RoutineBuilder() {
  ...
  const handleAddTemplate = (template: ActivityTemplate) => {
    const newActivity: Activity = {
      ...
      type: template.type,
      durationMinutes: 1, // default
      restSeconds: 30, // default
  ...
  const handleSaveRoutine = () => {
    if (!routineTitle.trim() || selectedActivities.length === 0) {
      alert('Please provide a title and add at least one activity.');
      return;
    }
    
    addRoutine({
      id: `r-${Date.now()}`,
      patientId: null, // Saves to general library
      title: routineTitle,
      type: routineType,
      completed: false,
      activities: selectedActivities.map((a, i) => ({ ...a, order: i + 1 })),
    });
  ...
  return (
    <div className="h-[calc(100vh-80px)] flex flex-col animate-fade-in">
      <header className="mb-6 flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-display font-bold text-on-surface mb-2">Routine Builder</h1>
          <p className="text-on-surface-variant font-body">Create new templates for your library.</p>
        </div>
        <Button onClick={handleSaveRoutine} className="flex gap-2 items-center">
      ...
      <div className="flex-1 flex gap-8 overflow-hidden pb-8">
        ...
        <div className="w-2/3 flex flex-col bg-surface-container-low rounded-2xl border-ghost overflow-hidden">
          <div className="p-4 border-b border-surface-container-high bg-surface-container-lowest space-y-3">
            ...
            {/* Filtro parte del cuerpo — solo cuando se ven físicos */}
            {catalogFilter !== 'BREATHING' && (() => {
              const usedParts = Array.from(new Set(
                templates
                  .filter(t => t.type === 'PHYSICAL' && t.bodyPart)
                  .map(t => t.bodyPart as BodyPart)
              ));
        ...
        <div className="flex-1 flex flex-col bg-surface-container-lowest rounded-2xl shadow-ambient border-ghost overflow-hidden">
          <div className="p-6 border-b border-surface-container-high bg-surface">
            <div className="flex gap-4 mb-4">
               <div className="flex-1">
                 <Input 
                   ...
                   placeholder="e.g. Post-Op Knee Mobility"
                 />
               </div>
               <div className="w-48">
                 <label className="text-sm font-body text-on-surface-variant ml-2 tracking-wide mb-1 block">Type</label>
                 <select 
                   value={routineType} 
            ...
            <p className="text-sm font-bold text-primary">{selectedActivities.length} Activities Added</p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-surface-bright">
            {selectedActivities.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-outline">
                ...
                <p className="font-display">Your routine is empty.</p>
                <p className="text-sm">Click the + on any video to add it here.</p>
              </div>
            ) : (
                ...
                <Card key={activity.id} level={2} className="relative p-0 overflow-hidden flex border-ghost">
                  ...
                  <div className="p-4 flex-1">
                     ...
                     <div className="grid grid-cols-3 gap-6">
                        <div>
                           <label className="text-xs text-on-surface-variant uppercase font-bold tracking-wider mb-1 block">Duration (min)</label>
                        ...
                        <div>
                           <label className="text-xs text-on-surface-variant uppercase font-bold tracking-wider mb-1 block">Rest (sec)</label>
                           <div className="flex items-center gap-2">
